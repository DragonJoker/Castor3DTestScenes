...
#define in_launcherCooldown 0.1
#define in_shellLifetime 1.0
#define in_secondaryShellLifetime 2.0
#define in_finalShellLifetime 250000.0
...
void main()
{
	...
	if ( l_gid < c3d_uiCurrentParticlesCount )
	{
		...
		case PARTICLE_TYPE_FINAL_SHELL:
			if ( l_age < in_finalShellLifetime )
			{
				float l_deltaTime = c3d_fDeltaTime / 1000.0;
				vec3 l_deltaP = vec3( l_deltaTime ) * l_velocity;
				vec3 l_deltaV = vec3( l_deltaTime ) * vec3( 0.0, -0.981, 0.0 );
