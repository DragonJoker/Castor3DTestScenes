...
void main (void)
{
	...
	vec4	l_v4Texture;
	vec3	l_v3Normal;
	vec4	l_v4Ambient = vec4( 0 );
	vec4	l_v4Diffuse = vec4( 0 );
	vec4	l_v4Specular = vec4( 0 );
...
//		l_v3Tmp			= l_light.m_v4Position.xyz - vtx_vertex;
		...
		l_v4Ambient		+= l_light.m_v4Ambient * c3d_v4MatAmbient * l_fAttenuation;
		l_v4Diffuse		+= l_light.m_v4Diffuse * c3d_v4MatDiffuse * max( dot( l_v3Light, l_v3Normal ), 0.0 ) * l_fAttenuation;
		l_v4Specular	+= l_light.m_v4Specular * c3d_v4MatSpecular * pow( clamp( dot( reflect( -l_v3Light, l_v3Normal ), l_v3EyeVec ), 0.0, 1.0 ), c3d_fMatShininess ) * l_fAttenuation;
	}

	out_FragColor = (l_v4Ambient * l_v4Texture + l_v4Diffuse * l_v4Texture + l_v4Specular);
}
