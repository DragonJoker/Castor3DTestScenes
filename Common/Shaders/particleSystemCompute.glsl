...
#define inLauncherCooldown 100.0
#define inShellLifetime 10000.0
#define inSecondaryShellLifetime 2500.0
...
void main()
{
	...
	if ( gid < c3d_currentParticlesCount )
	{
		...
		case PARTICLE_TYPE_LAUNCHER:
			...
			{
				age = 0.0;
				vec3 dir = GetRandomDir( c3d_totalTime ) * 5.0;
				dir.y = max( dir.y * 7.0, 10.0 );
		...
		case PARTICLE_TYPE_SHELL:
			if ( age < inShellLifetime )
			{
				...
				vec3 deltaV = vec3( deltaTime ) * vec3( 0.0, -0.981, 0.0 );
				EmitParticle( PARTICLE_TYPE_SHELL
					, position + deltaP
					, velocity + deltaV
					, age );
