...
vec4 ApplyLight( int p_iIndex, vec4 p_v4Ambient, vec3 p_v3EyeSpaceVertex, float p_fDot, vec2 p_v2DP, vec2 p_v2DS, float p_fD, vec2 p_v2UV, vec3 p_v3Tmp2, vec4 p_v4TextureRelief,  vec4 p_v4TextureColour )
{
	...
	l_v3Light			= (c3d_mtxModelView * l_light.m_v4Position).xyz;
	...
	l_v3Tmp1			= normalize( vec3( dot( l_v3LightDirection, vtx_tangent ), dot( l_v3LightDirection, vtx_bitangent ), l_fDot ) );
...
void main()
{
	...
	l_v2DP				= vtx_texture.xy * Tile;
	l_v2DS				= (normalize( vec3( dot( l_v3EyeSpaceVertex, vtx_tangent), dot( l_v3EyeSpaceVertex, vtx_bitangent), l_fDot ) ) * Depth / l_fDot).xy;
	...
	l_v4TextureRelief.xyz	= normalize( l_v4TextureRelief.x * vtx_tangent + l_v4TextureRelief.y * vtx_bitangent + l_v4TextureRelief.z * vtx_normal );
