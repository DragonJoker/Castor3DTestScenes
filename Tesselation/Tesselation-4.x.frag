...
void main()
{
	...
	{
		l_light			= GetLight( i );
		l_v3Position	= normalize( (te_mtxModelView * l_light.m_v4Position).xyz );
		l_fLambert		= dot( te_normal, l_v3Position );
	
		if( l_fLambert > 0.0 )
		{
			l_fSpecular		= pow( max( dot( reflect( -l_v3Position, te_normal ), l_v3Eye ), 0.0 ), c3d_fMatShininess );
