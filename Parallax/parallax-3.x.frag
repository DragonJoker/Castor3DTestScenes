...
void main()
{
    ...
    vec2 texUV, srcUV = vtx_texture;
    float height = texture2D( c3d_mapHeight, srcUV ).r;
    float v = height * Scale.x - Scale.y;
    vec3 eye = normalize( vtx_eyeVec );
	texUV = srcUV + (eye.xy * v);        

	vec4 rgb = texture2D( c3d_mapDiffuse, texUV );

	// output final color
	out_FragColor = vec4( rgb.xyz * height, rgb.a );
