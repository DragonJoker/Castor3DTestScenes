...
void main()
{
	...
	float height = texture2D(NormalMap, TexCoord).a;
	height = height * cBumpSize.x + cBumpSize.y;

	vec2 newUV = TexCoord + viewVec.xy * height;
	vec4 color_base = texture2D(DiffuseMap,newUV);
...
//	gl_FragColor = vec4(color_base.rgb * gl_LightSource[0].diffuse.rgb * (diffuse * base +0.7* specular * color_base.a) * LightAttenuation,1.0);
	gl_FragColor = vec4(color_base.rgb * ex_Light0Diffuse.rgb * (diffuse * base +0.7* specular * color_base.a) * LightAttenuation,1.0);
